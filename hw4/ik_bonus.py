...
def _compute_6d_error(target_pos, target_rot, current_pos, current_rot):
    """Compute 6D error (position + orientation) between two poses.
    ...
    tuple[np.ndarray, float]
        6D error vector and error norm.
    """
    # Position error
    pos_error = target_pos - current_pos
    
    # Orientation error (axis-angle from relative rotation)
    R_error = target_rot @ current_rot.T
    
    try:
        rot_error_obj = R.from_matrix(R_error)
        ori_error = rot_error_obj.as_rotvec()
    except Exception:
...
def _ik_pseudo_inverse(dh_params, target_pos, target_rot, base_pos, q_init, 
                       joint_limits, max_iters=1000, stop_thresh=0.001):
    """Solve IK using damped pseudo-inverse method.
...
def _ik_gradient_descent(dh_params, target_pos, target_rot, base_pos, q_init,
                        joint_limits, max_iters=1000, stop_thresh=0.001):
    """Solve IK using gradient descent with numerical Jacobian.
    ...
    tmp_q = q_init.copy()
    learning_rate = 0.1
    delta_q_step = 0.001  # Step for numerical differentiation
    
    for iteration in range(max_iters):
        current_pose, _ = your_fk(dh_params, tmp_q, base_pos)
        current_pos = np.asarray(current_pose[:3], dtype=np.float64)
        current_quat = np.asarray(current_pose[3:], dtype=np.float64)
        current_rot = R.from_quat(current_quat).as_matrix()
        ...
        for i in range(6):
            q_plus = tmp_q.copy()
            q_plus[i] += delta_q_step
            pose_plus, _ = your_fk(dh_params, q_plus, base_pos)
            pos_plus = np.asarray(pose_plus[:3], dtype=np.float64)
            quat_plus = np.asarray(pose_plus[3:], dtype=np.float64)
            rot_plus = R.from_quat(quat_plus).as_matrix()
            
            error_plus, _ = _compute_6d_error(target_pos, target_rot, pos_plus, rot_plus)
            gradient[i] = np.dot(error_6d, error_plus) / delta_q_step
        
        # Update joints in direction of negative gradient
        tmp_q = tmp_q - learning_rate * gradient
...
def _ik_transpose_jacobian(dh_params, target_pos, target_rot, base_pos, q_init,
                          joint_limits, max_iters=1000, stop_thresh=0.001):
    """Solve IK using Jacobian transpose method (simpler but less accurate).
    ...
    for iteration in range(max_iters):
        ...
        J = jacobian
        # Simple Jacobian transpose (faster but less accurate)
        J_transpose = J.T
        
        delta_q = J_transpose @ error_6d
        delta_q_norm = np.linalg.norm(delta_q)
        if delta_q_norm > 1e-6:
            delta_q = delta_q / delta_q_norm  # Normalize
...
def your_ik(new_pose : list or tuple or np.ndarray, 
    ...
    """Solve inverse kinematics using multiple methods.
    ...
    method : str, default="pseudo_inverse"
        IK method to use:
        - "pseudo_inverse": Damped pseudo-inverse (default, most accurate)
        - "gradient_descent": Gradient descent with numerical Jacobian
        - "levenberg_marquardt": Levenberg-Marquardt adaptive damping
        - "transpose_jacobian": Jacobian transpose (faster, less accurate)
    ...
    # Get DH parameters and target pose
    ...
    # Select IK method
    if method.lower() == "pseudo_inverse":
        tmp_q = _ik_pseudo_inverse(dh_params, target_pos, target_rot, base_pos, tmp_q,
                                   joint_limits, max_iters, stop_thresh)
    elif method.lower() == "gradient_descent":
        tmp_q = _ik_gradient_descent(dh_params, target_pos, target_rot, base_pos, tmp_q,
                                     joint_limits, max_iters, stop_thresh)
    elif method.lower() == "levenberg_marquardt":
        tmp_q = _ik_levenberg_marquardt(dh_params, target_pos, target_rot, base_pos, tmp_q,
                                       joint_limits, max_iters, stop_thresh)
    elif method.lower() == "transpose_jacobian":
        tmp_q = _ik_transpose_jacobian(dh_params, target_pos, target_rot, base_pos, tmp_q,
                                      joint_limits, max_iters, stop_thresh)
    else:
        raise ValueError(f"Unknown IK method: {method}. Use 'pseudo_inverse', 'gradient_descent', "
                        "'levenberg_marquardt', or 'transpose_jacobian'.")
...
if __name__=="__main__":
    ...
    parser.add_argument('--method', type=str, default='pseudo_inverse',
                        choices=['pseudo_inverse', 'gradient_descent', 'levenberg_marquardt', 'transpose_jacobian'],
